...
export interface Interest {
  ...
  description: string;
  icon: string;
  category: 'Recommended' | 'Trending' | 'Latest';
  color: string;
}
export const INTERESTS: Interest[] = [
  { id: '1', name: 'Social Media', description: 'TikTok, Meta, X', icon: 'brand_family', category: 'Recommended', color: 'blue' },
  { id: '2', name: 'Politics', description: 'Global & Local', icon: 'gavel', category: 'Recommended', color: 'slate' },
  { id: '3', name: 'Technology', description: 'AI, Gadgets, Apps', icon: 'memory', category: 'Recommended', color: 'indigo' },
  { id: '4', name: 'Sports', description: 'NBA, Soccer, F1', icon: 'sports_soccer', category: 'Recommended', color: 'green' },
  { id: '5', name: 'Entertainment', description: 'Movies, Music, Celebs', icon: 'movie', category: 'Trending', color: 'pink' },
  { id: '6', name: 'Business', description: 'Stocks, Finance, Startups', icon: 'payments', category: 'Trending', color: 'emerald' },
  { id: '7', name: 'Science', description: 'Space, Biotech, Climate', icon: 'science', category: 'Trending', color: 'orange' },
  { id: '8', name: 'Gaming', description: 'PC, Console, Esports', icon: 'sports_esports', category: 'Trending', color: 'purple' },
  { id: '9', name: 'Health', description: 'Fitness, Wellness, Meds', icon: 'vital_signs', category: 'Latest', color: 'red' },
  { id: '10', name: 'Lifestyle', description: 'Travel, Food, Fashion', icon: 'style', category: 'Latest', color: 'amber' },
  { id: '11', name: 'World News', description: 'International Affairs', icon: 'public', category: 'Latest', color: 'sky' },
];



function InterestsPage() {
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set(['1', '4', '11']));
  ...
  const selectedList = INTERESTS.filter(i => selectedIds.has(i.id));
  ...
  return (
    <div className="min-h-screen pb-32">
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-12">
        ...
        {/* Categories */}
        <div className="mb-10">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-6">Popular Categories</p>
          ...
          {filteredInterests.map(item => {
            ...
            return (
              <div
                ...
                className={`relative group cursor-pointer aspect-[4/3] rounded-2xl border-2 p-6 flex flex-col justify-between transition-all hover:shadow-xl ${isSelected ? 'border-primary bg-primary/5 shadow-lg shadow-primary/10' : 'border-slate-100 bg-white hover:border-primary'
                  }`}
              >
                <div className="flex justify-between items-start">
                  <div className={`p-4 rounded-xl shadow-sm transition-colors ${isSelected ? 'bg-white text-primary' : 'bg-slate-50 text-slate-600 group-hover:text-primary group-hover:bg-slate-100'
                    }`}>
                    <span className="material-symbols-outlined text-3xl font-bold">{item.icon}</span>
                  </div>
                  {isSelected && (
                    <span className="material-symbols-outlined text-primary font-black animate-in zoom-in duration-200">check_circle</span>
                  )}
                </div>
                <div>
            ...
            <div className="flex -space-x-3">
              {selectedList.slice(0, 3).map((item, idx) => (
                <div
                  ...
                  style={{ backgroundColor: item.color === 'blue' ? '#137fec' : item.color === 'green' ? '#10b981' : '#38bdf8' }}
                >
